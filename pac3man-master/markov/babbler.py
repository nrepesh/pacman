...
class Babbler:
    ...
    def add_sentence(self, sentence):
        ...
        """
        sentence = sentence.lower() + ' EOL'
        n = self.n
        states = []
        words = sentence.split()
        # Starters and stoppers
        self.starters.append(words[0:n])
        self.stoppers.append(words[-n-1:len(words)-1])

        # Creates states and appends it
        ...
        for i in range(len(states) - 1):
            ...
            if 'EOL' in key.split():
                break
            if key not in self.all_dict.keys() and len(states[i+1]) != 0:
                self.all_dict[key] = []
                self.all_dict[key].append(states[i + 1][-1])
    ...
    def get_starters(self):
        ...
        for i in range(len(self.starters)):
            word = ' '.join(self.starters[i])
            self.final_startes.append(word)
        #print(self.final_startes)
        return self.final_startes
    ...
    def get_stoppers(self):
        ...
        multiple sentences.
        """
        for i in range(len(self.stoppers)):
            word = ' '.join(self.stoppers[i])
            self.final_stoppers.append(word)
        #print(self.final_stoppers)
        return self.final_stoppers
    ...
    def get_successors(self, ngram):
        ...
        """
        #print(self.all_dict[ngram])
        return self.all_dict[ngram]
    ...
    def get_all_ngrams(self):
        ...
        """
        #print(list(self.all_dict.keys()))
        return list(self.all_dict.keys())
    ...
    def get_random_successor(self, ngram):
        ...
        """
        num = random.randint(0, len(self.all_dict[ngram])-1)
        return self.all_dict[ngram][num]
    ...
    def babble(self):
        ...
        """
        sen = ''
        start = ' '.join(self.starters[random.randint(0, len(self.starters)-1)])
        sen = sen + start

        while 'EOL' not in start and self.get_random_successor(start) != 'EOL':
            sen = sen + ' ' + self.get_random_successor(start)
            n = self.n
            start = ' '.join(sen.split()[-n:len(sen)-1])
        # if EOL is found then remove it from output
        if sen.split()[-1] == 'EOL':
            sen = ' '.join(sen.split(' ')[:-1])
        return sen.capitalize()  #Capitalizes the statement
...
def main(n=3, filename='tests/test1.txt', num_sentences=5):
    ...
    '''
    #For testing
    #babbler.get_starters()
    #babbler.get_stoppers()
    #babbler.get_successors()
    #print(babbler.get_all_ngrams())
    #print(babbler.has_successor('lord our god'))
    #print(babbler.all_dict)
    #print(babbler.get_random_successor('of the lord'))
...
if __name__ == '__main__':
    ...
    sys.argv.pop(0)
    n = 5
    filename = 'tests/books/austen-sense.txt'
    num_sentences = 5
    if len(sys.argv) > 0:
